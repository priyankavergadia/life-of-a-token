...
const LiveErr = ({ msg }) => <div className="live-err">⚠️ {msg}</div>
const LiveBadge = () => <span className="live-badge">● LIVE</span>

function NeedKey({ connected, children }) {
  if (connected) return children
  return <div className="need-key">🔌 Connect a provider above to run this for real. Showing the demo for now.</div>
}
...
function ConnectBar({ creds, setCreds }) {
  ...
  return (
    <div className="connect-bar">
      <div className="cb-row">
        ...
        <input
          className="cb-key" type="password" autoComplete="off" spellCheck={false}
          placeholder={`paste your ${p.label} key (${p.keyHint})`}
          value={key}
      ...
      <div className="cb-note">🔒 Your key is stored <b>only in this browser</b> and sent <b>directly to {p.label}</b> — it never touches our servers. {!p.embeds && <span> · note: Claude has no embeddings API (the Embeddings lab needs Gemini or OpenAI).</span>}</div>
    </div>
  )
}
...
export default function LabJourney() {
  ...
  const liveCreds = { provider: creds.provider, key: creds.keys[creds.provider] || '', connected: !!creds.keys[creds.provider] }
  ...
  return (
    <>
      <div className="journey-head">
        <p className="journey-tag">A click-through tour of a real GenAI lab — see the code <b>and</b> run it live.</p>
      ...
      <div className="lab-stage">
        ...
        <motion.div key={cur.id + '-v'} className="panel viz lab-visual" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
          <div className="panel-cap">✨ What it does {liveCreds.connected && <span className="cap-live">· live on {PROVIDERS[liveCreds.provider].label}</span>}</div>
          <Visual creds={liveCreds} />
